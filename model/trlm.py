...
class TRLM(nn.Module):
    def __init__(self, config: TRLMConfig) -> None:
        super().__init__()
    ...
    def _run_blocks(
        ...
        x = x + input_injection
        for layer in self.transformer.h:  # type: ignore
            x = layer(
                x,
                cos=cos,
                sin=sin,
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: TRLMConfig, block_idx: int) -> None:
        super().__init__()
...
class GptNeoxMLP(nn.Module):
    def __init__(
        self, config: TRLMConfig, intermediate_size: Optional[int] = None
    ) -> None:
        super().__init__()
...
def build_rope_cache(
    ...
    Args:
        ...
        extra_config (dict, optional):TRLMConfiguration parameters for frequency adjustments (used by Llama 3.1 and 3.2)
